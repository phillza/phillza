...
- **[ml-leakage-guard](https://github.com/phillza/ml-leakage-guard)** — `leakage-guard` CLI + library. Static-analysis linter for 9 common ML data-leakage patterns (rolling without `shift(1)`, test data in `eval_set`, hardcoded `date.today()`, etc.) plus three small pandas helpers. Pre-commit ready.
